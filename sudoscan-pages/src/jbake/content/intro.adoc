...
The model was created using Python/Keras/Tensorflow, but it's possible to use the trained model in the JVM world. By the
...
approach (and more mathematical way) to solve this kind of problem is to model it as an Operations Research (OR) Problem, and use a Mixed Integer Programming (MIP) solver to find the optimal solution. A comparison between these two approaches can be found in this
